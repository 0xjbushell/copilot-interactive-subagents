...
export function createDefaultAgentLaunchCommand(request = {}, runtimeServices = {}, { agentIdentifier, task, copilotSessionId, interactive, backend, model }) {
  ...
  const runnerScript = [
    ...
    'const closePaneOnce = () => {',
    '  if (closePaneOnce.fired) return; closePaneOnce.fired = true;',
    '  try {',
    '    const zPane = process.env.ZELLIJ_PANE_ID;',
    '    const tPane = process.env.TMUX_PANE;',
    '    if (zPane) {',
    '      spawnSync("zellij", ["action", "close-pane", "--pane-id", zPane], { stdio: "ignore", timeout: 2000 });',
    '    } else if (tPane) {',
    '      spawnSync("tmux", ["kill-pane", "-t", tPane], { stdio: "ignore", timeout: 2000 });',
    '    }',
    '  } catch (_) {}',
    '};',
